...
class Kinopoisk:
    ...
    def __get(self, url: str, params: dict = None, token: str = None) -> requests.Response:
        ...
        return requests.get(url=full_url, params=params, headers={"X-API-KEY": token}, timeout=15)

    def movie_random(self, token: str = None) -> requests.Response:
        return self.__get(url=self.__urls.get("random"), token=token)

    def check_response_code(self, response: requests.Response, expected_code: int):
        if response.status_code != expected_code:
            pytest.fail(f'Response "{response.url}" status code({response.status_code}) != expected_code({expected_code})')
    ...
    def check_response_body(self, response: requests.Response, schema_name: str, nested_keys: list = None):
        ...
        if not self.__schema_checker.check_field_keys(schema_name=schema_name, data=response.json(), nested_keys=nested_keys):
